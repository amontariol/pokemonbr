...
class play:
    def __init__(self):
        ...
        tl=15#temps restant à l'utilisateur pour choisir son pokémon
        while tl>0:#donne 15 secondes au joueur pour choisir son pokémon de départ
            ...
            pygame.draw.rect(self.win,(255,0,0),((50,70, (412/15)*tl, 30)))
        ...
        c.execute("INSERT INTO pokemon VALUES ("+npokemon+","+str(base[0])+",'"+base[1]+"','"+base[2]+"',5,"+str(base[3])+","+str(base[3])+","+str(base[4])+","+str(base[5])+","+str(base[6])+","+str(base[7])+","+team+")")#Attribue les données génériques a notre pokemon de départ
                    ...
                    if int(self.xft)==self.pnjs[i].pos[0]/32 and int(self.yft)==self.pnjs[i].pos[1]/32:
                        if self.pnjs[i].active:
                            if self.pnjs[i].trainer:
                                ...
                                self.message=["Prend ces "+str(quant)+str(objet),"Tu en aurra plus besoin que moi"]
    ...
    def pickpokeball(self,ft):#récupere la pokeball au sol et attribue les items
        ...
        print("Vous avez trouvé ",quant,objet)
...
class createpnj:
    ...
    def createteam(self,typ,lvl):
        ...
        for row in c.execute("SELECT * FROM pokemon"):
            npokemon+=1
        npokemon=str(npokemon)
        c.execute("INSERT INTO pokemon VALUES ("+npokemon+","+str(base[0])+",'"+base[1]+"','"+base[2]+"',"+str(lvl)+","+str(base[3])+","+str(base[3])+","+str(base[4])+","+str(base[5])+","+str(base[6])+","+str(base[7])+","+team+")")#+","+str(base[7])######Attribue les données génériques a notre starter 
        nattack=1
        for row in c.execute("SELECT * FROM attaquespokemon"):
            nattack+=1
    ...
    def __init__(self,fen,pnjo):
        ...
        if self.pnj!=False:
            ...
            for row in c.execute("SELECT * FROM basepokemon WHERE id="+str(poke)):
                print(row)
            self.Bteam.append([0,row[0],row[1],row[2],abs(int(random.randint(moy-10,moy)))+1,row[3],row[3],row[4],row[5],row[6],row[7],joueur.numteam,[[2, 'Absorb', 20, 100, 'grass', 10]]])#gen du pokemon a randomiser
        conn.close()     
    ...
    def fight(self):
            ...
            if played==True:#quand le joueur à joué
                if attack==True:#si le joueur à choisi d'attaquer
                    if self.Pteam[0][7]>=self.Bteam[0][7]:
                        self.wmessage("Vous infligez "+str(self.Pteam[0][12][int(self.wmenu[6])-1][2])+" points de dégats")
                        self.Bteam[0][5]-=self.Pteam[0][12][int(self.wmenu[6])-1][2]
                        if self.Bteam[0][5]>0:
        ...
        if run:
            ...
            if self.Bteam[0][5]==0:
                ...
                self.Pteam[0][4]+=1
                self.Pteam[0][6]=int(self.Pteam[0][6]*1.2)
