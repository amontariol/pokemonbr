...
class Launcher:
    ...
    def verifierlogin(self):
        ...
        for row in c.execute("SELECT * FROM joueur"):#boucle for permettant de selectionner les element de joueur et de les stocker dans leurs listes respectives
            ...
            logins.append(row[4])
        stop=False
        for i in range (0,len(logins)-1):#boucle for permettant de parcourir login et mdp et de verifier si les valeurs encdées par l'utilisateur sont présentes dedans.s
    ...
    def creer_menus(self):
        ...
        submenu.add_command(label='stats', command=lambda:webbrowser.open_new('http://localhost/SITE%20TFE%20Final/Vue/wiki.php'))#onglet de pages 
        ...
        jeu.add_command(label='Quit', command=self.fen.destroy)#ajout du bouton quitter
    ...
    def creer_start(self,login):
        ...
        Label(self.fen, text="Bienvenue,",bg="#4b433b",font="Broadway").place(x=self.width-237, y=250)#placemement de Labels
        Label(self.fen, text=login,bg="#4b433b",font="Broadway").place(x=self.width-237, y=300)#placemement de Labels
        Label(self.fen, text="le jeu est pret à être lancé",bg="#4b433b",font="Broadway").place(x=self.width-237, y=350)#placemement de Labels

        Button(self.fen, width=20, text="Play",bg="#4b433b",font="Broadway", relief=GROOVE, command=lambda:launchgame(self.fen,login)).place(x=self.width-237, y=680)#placemement de Bouttons
    ...
    def creer_launcher(self):
        ...
        self.l1txt = Label( self.fen,text = "Login:",width=15,bg="#4b433b",font="Broadway" ).place(x=self.width-200, y=125)#placemement de Labels
        self.l1entree = Entry( self.fen,width=17,textvariable = self.login,bg="#4b433b",font="Broadway", relief=GROOVE ).place(x=self.width-200, y=175)#placemement de Labels

        Label( self.fen,text = "Mot de Passe:",width=15,bg="#4b433b",font="Broadway" ).place(x=self.width-200, y=225)#placemement de Labels
        Entry( self.fen,width=17,textvariable = self.mdp, show="*",bg="#4b433b",font="Broadway", relief=GROOVE ).place(x=self.width-200, y=275)#placemement de Labels

        Button(self.fen, text="Login", width=7,bg="#4b433b",font="Broadway", command=lambda: self.verifierlogin(), relief=GROOVE).place(x=self.width-150, y=325)#placemement de Bouttons


        self.fen.mainloop
        
    
def launchgame(fen,login):
    """
    methode permettant de lancer le jeu
    prend fen en parametre en ne renvoie rien
    """
    fen.destroy()#destruction de la fenetre
    PokemonBr.jouer(login)#appel du jeu principal
